...
- 已执行 `autorouter-cli connect <port>` 或设置 `AUTOROUTER_URL` 环境变量
...
# 列出所有实例
autorouter-cli list
autorouter-cli list --json

# 创建实例
autorouter-cli create --id dev --mode attached --browser-url http://localhost:9222
autorouter-cli create --id managed1 --mode managed

# 启动/停止/重启
autorouter-cli start <id>
autorouter-cli stop <id>
autorouter-cli restart <id>

# 切换默认实例
autorouter-cli switch <id>

# 查看状态
autorouter-cli status <id>

# 删除实例
autorouter-cli delete <id>
```
...
```bash
# 获取默认实例的 wsEndpoint
autorouter-cli get-ws

# 获取指定实例的 wsEndpoint
autorouter-cli get-ws dev
...
autorouter-cli --port 9300 get-ws dev
```
...
autorouter-cli skills get autorouter-cli     # 输出本 skill 完整内容
