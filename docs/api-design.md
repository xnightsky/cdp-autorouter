...
4. **请求路径双触发器自愈**（2026-09-20 起覆盖显式实例路径）：managed 实例采用双触发器架构保证“端口可用”语义——触发器 A（`resolveInstance` 检测到 managed 实例非 healthy 时主动交 `supervisor.start`）+ 触发器 B（fetch 失败后兜底，attached 返 503，managed retry）；`POST /api/instances/{id}/restart` 仍可用但已非唯一恢复途径；另有 server 端低频巡检（HealthMonitor，见架构文档 §9.4）在无请求时自动恢复 managed 实例；
5. **attached 不自愈**：任何路径上 attached 不可达也仅返回 503 诊断信息，autorouter 不擅自启动外部 chrome；
6. **响应码分路径语义**：
   ...
   | 显式路径 + managed | 链路通畅（含触发器 A 自愈后） | 启动超时或触发器 B retry 失败 | autorouter 自身 bug |
   | 显式路径 + attached | 链路通畅 | 触发器 B 检测到上游不可达 | autorouter 自身 bug |
